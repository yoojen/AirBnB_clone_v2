...
def do_deploy(archive_path):
    ...
    if put(archive_path, upload_path).failed is True:
        return False
    if run('mkdir -p /data/web_static/releases/' + filename + '/\
           ').failed is True:
        return False
    # uncompressing to other folader
    if run('tar -xvzf' + upload_path + ' -C /data/web_static/releases/\
           ' + filename).failed is True:
        return False
    # deleting compressed file from server
    if run('rm -rf ' + upload_path).failed is True:
        return False
    if run("mv /data/web_static/releases/" + filename + "/web_static/* \
           /data/web_static/releases/" + filename + "/").failed is True:
        return False
    if run("rm -rf data/web_static/releases/" + filename + "\
            /web_static").failed is True:
        return False
    # deleting symbolic link
    if run("rm -rf /data/web_static/current").failed is True:
        return False
    if run("ln -s /data/web_static/releases/" + filename + "/\
